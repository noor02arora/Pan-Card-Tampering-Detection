...
def index():
    ...
    if request.method == 'POST':
        ...
        original_img = imread(os.path.join(
            app.config['EXISTING_FILE'], 'original.jpg'))
        test_img = imread(os.path.join(
            app.config['INITIAL_FILE_UPLOADS'], 'test_image.jpg'))
        ...
        score, diff_img = structural_similarity(
            original_img, test_img, full=True)
        ...
        Image.fromarray(diff_img).save(os.path.join(
            app.config['GENERATED_FILE'], 'diff_image.jpg'))
